...
class QuedisApp:
    ...
    def create_database(self):
        try:
            excel_table = r'C:\\student_excel_table.xlsx'
            data = pd.read_excel(excel_table, header=None)
            data.columns = ['name']
    ...
    def view_group(self):
        ...
        cur.execute('''SELECT name FROM StudentScores''')
        results = cur.fetchall()
        conn.close()

        students = [row[0] for row in results]
        messagebox.showinfo("Список студентов", "\n".join(students))
